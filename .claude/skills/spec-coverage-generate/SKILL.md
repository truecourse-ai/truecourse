...
1. **No GitHub event fired you.** Ask the user for: the **local path** to the spec folder (a dir of
   `.md` files) and a short **group name**. Do not read `groups.yaml` or pick a `pending` campaign —
   the user names the group and points at the docs.
...
Report where the `.tc` contracts landed and the artifact counts (`node dist/cli.mjs contracts list`
against the working dir). Then tell the user to run **`/spec-coverage-measure`** on the same group
to score coverage and surface any missing kinds.
