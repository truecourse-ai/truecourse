...
export type GraphLevel = 'service' | 'module' | 'method' | 'all';
...
// ---------------------------------------------------------------------------
// All-level graph: returns the full hierarchy for semantic zoom
// ---------------------------------------------------------------------------

export interface AllLevelGraphData {
  services: Array<{
    id: string;
    name: string;
    type: string;
...
export function buildAllLevelGraph(input: UnifiedInput): AllLevelGraphData {
  const {
    services, serviceDeps, layers, modules, moduleDeps,
    methods, methodDeps, databases, dbConnections,
  ...
  // Build violation count by module name
  const violationCountByModule = new Map<string, number>();
  if (deterministicViolations) {
    for (const v of deterministicViolations) {
      if (v.moduleName) {
        violationCountByModule.set(v.moduleName, (violationCountByModule.get(v.moduleName) || 0) + 1);
      }
    }
  }

  // Build directory grouping: group modules by directory within each layer
  const modulesByLayer = groupBy(modules, (m) => m.layerId);
  const methodsByModule = groupBy(methods, (m) => m.moduleId);

  const dirList: AllLevelGraphData['directories'] = [];
  const moduleList: AllLevelGraphData['modules'] = [];
  const dirIdMap = new Map<string, string>(); // moduleId -> directoryId

  for (const layer of layers) {
    const layerModules = modulesByLayer.get(layer.id) || [];
    // Group by directory path (parent folder of file)
    const byDir = new Map<string, typeof layerModules>();
    for (const mod of layerModules) {
      const parts = mod.filePath.split('/');
      parts.pop(); // remove filename
      const dirPath = parts.join('/') || '.';
      if (!byDir.has(dirPath)) byDir.set(dirPath, []);
      byDir.get(dirPath)!.push(mod);
    }

    for (const [dirPath, dirModules] of byDir) {
      const dirId = `dir__${layer.id}__${dirPath}`;
      let violationCount = 0;
      for (const mod of dirModules) {
        violationCount += violationCountByModule.get(mod.name) || 0;
      }
      ...
      dirList.push({
        ...
        layerId: layer.id,
        dirPath,
        moduleCount: dirModules.length,
        violationCount,
      });

      for (const mod of dirModules) {
        dirIdMap.set(mod.id, dirId);
  ...
  // Pre-aggregate service-level edges
  const svcEdgeMap = new Map<string, { count: number; type: string | null }>();
  for (const dep of serviceDeps) {
    const key = `${dep.sourceServiceId}::${dep.targetServiceId}`;
    const existing = svcEdgeMap.get(key);
    if (existing) {
      existing.count += dep.dependencyCount || 0;
    } else {
      svcEdgeMap.set(key, { count: dep.dependencyCount || 0, type: dep.dependencyType || null });
    }
  }
  const serviceEdges: AllLevelGraphData['edges']['service'] = [];
  let seIdx = 0;
  for (const [key, val] of svcEdgeMap) {
    const [source, target] = key.split('::');
    serviceEdges.push({ id: `svc-edge-${seIdx++}`, source, target, count: val.count, type: val.type });
  }
  ...
  return {
    services: services.map((s) => {
      const svcLayers = layersByService.get(s.id) || [];
      return {
        id: s.id,
        name: s.name,
        type: s.type,
    ...
    layers: layers.map((l) => ({
      ...
      filePaths: l.filePaths,
      layerColor: LAYER_COLORS[l.layer] || '#6b7280',
    })),
    directories: dirList,
    modules: moduleList,
    ...
    edges: {
      service: serviceEdges,
      module: moduleDeps.map((d) => ({
        id: d.id,
        source: d.sourceModuleId,
        target: d.targetModuleId,
        count: d.dependencyCount,
      })),
      method: methodDeps.map((d) => ({
        id: d.id,
        source: d.sourceMethodId,
        target: d.targetMethodId,
        count: d.callCount,
      })),
    },
    ...
    dbConnections: dbConnections.map((c) => ({
      serviceId: c.serviceId,
      databaseId: c.databaseId,
      driver: c.driver,
    })),
    violations: (deterministicViolations || []).map((v) => ({
      id: v.id,
      ruleKey: v.ruleKey,
      category: v.category,
      title: v.title,
