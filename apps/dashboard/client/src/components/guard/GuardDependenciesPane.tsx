 ...
 * URL-synced as `?gext=<name>`. So a row is a link a teammate can be sent — and the
...
import { GuardDependencyDetail } from './GuardDependencyDetail';
...
export interface GuardDependenciesPaneProps {
  ...
  reloadKey?: number;
  /** Jump into the Tests tab with one flow's detail open (`?gflow=`). */
...
export function GuardDependenciesPane({ repoId, reloadKey = 0, onOpenFlow }: GuardDependenciesPaneProps) {
  ...
  const { activeId, openTabs, open, close } = useGuardTabs('gext', repoId);
