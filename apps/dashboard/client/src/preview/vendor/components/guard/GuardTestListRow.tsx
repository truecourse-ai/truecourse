...
import type { GuardFlowPlainStatus } from '@/preview/vendor/lib/guard-flow-status';
  ...
  return (
    <>
      <span className="flex w-full min-w-0 items-center gap-2">
        <span className="min-w-0 flex-1 truncate text-[13px] font-normal leading-snug text-foreground">{row.title}</span>
        <GuardFlowStatusChip status={row.status.plain} word={row.status.word} />
      </span>
      <div className="flex w-full min-w-0 flex-wrap items-center gap-1 empty:hidden">
        {row.handWritten && (
