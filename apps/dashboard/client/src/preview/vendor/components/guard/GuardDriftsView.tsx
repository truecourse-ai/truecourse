...
// Copied from the agentic branch's apps/dashboard/client/src/components/guard/GuardDriftsView.tsx; delete with the preview.
/**
...
import { orderGuardDrifts } from '@/preview/vendor/lib/guard-drifts';
import { GuardRunSummary } from '@/preview/vendor/components/guard/GuardRunSummary';
...
import { GuardTabStrip } from '@/preview/vendor/components/guard/GuardTabStrip';
...
export function GuardDriftsView({
  ...
  const { activeId, openTabs, open, close } = useGuardTabs('result', repoId);
    ...
    <div className="flex h-full w-full overflow-hidden">
      ...
      <CollapsibleAside label="Run" defaultWidth={288}>
        <div className="h-full min-h-0 overflow-hidden bg-card">
          <GuardRunSummary repoId={repoId} run={run} history={history} selectedRunId={selectedRunId} onSelectRun={selectRun} />
        </div>
      </CollapsibleAside>
