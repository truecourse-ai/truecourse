...
// Copied from the agentic branch's apps/dashboard/client/src/components/spec/SpecProgressPopup.tsx; delete with the preview.
/**
 ...
 * `spec:progress` Socket.io event. {@link SpecProgressSteps} is the body alone,
 * for a surface that must show the same stream inline (the add-source dialog,
 * which sits above the floating popup's z-plane).
 */

import { Check, CircleX, Loader2, X } from 'lucide-react';
import type { AnalysisProgress } from '@/preview/vendor/hooks/useSocket';

/** The stream's body: the error, the checklist, or the single running line. */
export function SpecProgressSteps({ progress }: { progress: AnalysisProgress }) {
