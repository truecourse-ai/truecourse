...
// Copied from the agentic branch's apps/dashboard/client/src/components/spec/SpecCorpusView.tsx; delete with the preview.
/**
...
import { buildCorpusConflicts, isConflictId } from '@/preview/vendor/shared';
import type { SpecCorpusResponse, SpecCorpusDoc, SpecConflictResolution, SpecDecisionAck, SpecSkippedDoc } from '@/preview/vendor/lib/api';
        ...
        renderRow={(row) =>
          row.kind === 'conflict' ? (
            <OverlapRowContent
              ...
              change={data.version?.conflictChanges[row.id]}
            />
          ) : row.kind === 'doc' ? (
            <DocRowContent
              doc={row.doc}
 ...
 */
/** The version diff marker a row wears: what this version did to it against its parent. */
function ChangeMark({ change }: { change: 'added' | 'edited' | 'removed' | 'opened' | 'resolved' | undefined }) {
  if (!change) return null;
  const dot =
    change === 'added' || change === 'resolved'
      ? 'bg-emerald-500'
      : change === 'removed'
        ? 'bg-red-500'
        : 'bg-sky-500';
  ...
  return (
    <div className="group flex w-full min-w-0 flex-col gap-0.5 text-[13px]">
      <span className="flex w-full min-w-0 items-center gap-2">
        ...
        <HoverPopover content={disabledReason ?? 'Exclude this doc from the corpus'} side="top" align="end">
          <button
            ...
            className="shrink-0 rounded px-1.5 py-0.5 text-[10px] text-muted-foreground opacity-0 transition-opacity hover:bg-muted hover:text-foreground focus:opacity-100 group-hover:opacity-100 disabled:opacity-50 disabled:hover:bg-transparent"
          >
            skip
          </button>
        </HoverPopover>
      ...
      <span className="flex w-full min-w-0 items-center gap-1 text-[11px] text-muted-foreground">
        ...
        {tags.length > 2 && (
          <span className={CHIP_CLASS}>+{tags.length - 2} more</span>
        )}
        <span className="ml-auto flex shrink-0 items-center gap-2">
          <ChangeMark change={change} />
          {formatRelativeTime(doc.lastTouched)}
        </span>
      </span>
    </div>
  );
...
function OverlapRowContent({
  ...
  workspace?: boolean;
  /** What the viewed version did to this conflict against its parent. */
  change?: 'opened' | 'resolved';
}) {
  return (
    <div className="flex w-full min-w-0 flex-col gap-0.5 text-[13px]">
      <span className="flex w-full min-w-0 items-center gap-2">
        <span className="min-w-0 flex-1 truncate text-foreground">{label}</span>
        <span className="inline-flex shrink-0 items-center gap-1.5 text-[10px] font-medium text-foreground">
          <span aria-hidden className={`h-2 w-2 shrink-0 rounded-full ${resolved ? 'bg-emerald-500' : 'bg-amber-500'}`} />
          {resolved ? 'Resolved' : 'Open'}
        </span>
      </span>
      <span className="flex w-full min-w-0 items-center gap-1 text-[11px] text-muted-foreground">
        <span className={CHIP_CLASS}>{area}</span>
        {workspace && <WorkspaceBadge />}
      </span>
    </div>
  );
