...
// Copied from the agentic branch's packages/shared/src/guard/scenario.ts; delete with the preview.
/**
...
import { z } from 'zod'
import { capturedNamesIn } from './capture'
import type { GuardStepActual } from './step-actuals'
import {
  milestoneClaims,
  milestoneOrder,
  type GuardStepKind,
...
import { guardDriverIds, type GuardDriverId } from './drivers'
import {
  GuardWebStepSchema,
  describeWebCommand,
  describeWebExpect,
...
function dropLegacyDriverKey(value: unknown): unknown {
  ...
  if (!('driver' in value) && !('doc' in value)) return value
