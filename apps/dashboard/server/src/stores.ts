 ...
 * `@truecourse/data-store` Postgres impls, so the whole pipeline — the project
 * registry, specs, the workspace's context, guard state and the LLM-stage
 * caches — reads and writes the database instead of a repo's `.truecourse/`
 * tree. The working tree becomes an ephemeral per-run clone (see
 * services/run-clone.service.ts).
 *
 * Called once at boot, right after the migrations run.
 */
