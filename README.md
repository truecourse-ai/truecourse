...
**Generation runs as agent sessions.** Claim extraction (one session per doc), flow synthesis (one per area, plus one epic pass) and the flow workers (one per changed flow, which author the flow's scenarios and prove each draft by running it) are multi-turn sessions on the configured driver, every one of them transcribed under `.truecourse/sessions/guard-generate/<runId>/` and watchable live in the dashboard's Activity view. A worker's green draft is reviewed by a fidelity child before it is committed — does the test actually verify the claim it binds to? — and a session that dies transport-class fails its item loudly, never silently. Each step can be run alone (`--only-extract | --only-flows | --only-worker`), prior steps replaying from their outcome caches.

**4. Guard run** (`truecourse guard run`) — Fully deterministic: builds the repo via the recipe, executes every committed scenario — including the ones that were already failing at birth — across every surface a scenario's steps touch (cli commands, `request` steps against the served api, web steps in a real browser through `playwright-core`) and writes the run to `.truecourse/guard/` (per-run snapshots, `LATEST.json` as the merged board, per-scenario evidence bundles: the transcript plus, for a browser run, per-step screenshots and the session video). A test that was red at birth simply comes back green once the code catches up. A scenario that binds a supplied dependency nobody registered settles `blocked`, naming it, instead of running against a stand-in. Exits non-zero on any drift, so it drops straight into CI. No LLM, no API key, no `claude` binary — the one opt-in exception is the visual judge (`TRUECOURSE_GUARD_VISUAL_JUDGE=1`), which shows a failing web step's screenshot to a vision model and records what it saw beside the failure; it never moves an outcome.

**5. Guard adjudicate** (`truecourse guard adjudicate`) — After a run, one agent session per failing scenario reads its evidence and classifies it: a **bug** (the code is wrong), a **drift** (the doc is wrong) or a **test defect** (ours). Verdicts land on the board and `--report` renders them as `guard/findings.md`, numbered stably across runs.
...
truecourse guard adjudicate                       # Classify the board's failures with an agent session each: bug, doc drift, or test defect (--run <id>, --scenario <id>)
truecourse guard adjudicate --report              # Render guard/findings.md from the board's bug/drift verdicts
truecourse guard generate --only-extract          # One session step in isolation (--only-flows, --only-worker); --from-scratch re-authors changed flows
