...
> **Prerequisite:** the spec scan, guard setup and the guard generator shell out to the Claude Code CLI (`claude -p`). Install Claude Code and sign in once before running `spec scan`, `guard setup` or `guard generate`. `guard run` needs neither — it's deterministic.
