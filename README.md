...
- **Agent** — the agent's own page: every piece of work it did across the workspace's repositories (a spec scan, a guard setup, a generate), newest first, narrowed by kind, status and repository, each opening as one conversation rendered verbatim from the stored transcript: the steps of the work as headings, and under them every message of every inner session in order, the system prompt, every user message (the briefing and the loop's own interventions), every model turn with its text and its tool call, every tool result, the outcome value, and the loop's events as their fields. Nothing is paraphrased by the client. A live piece of work tails as it happens. Served by `GET /api/sessions/runs` (workspace-wide, `?repo=&kind=&status=&limit=&before=`), `GET /api/sessions/runs/:runId`, and the per-repository history pages `GET /api/repos/:id/sessions/runs/:command/:runId/activity?after=&limit=`.
