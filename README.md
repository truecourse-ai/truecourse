...
Claim extraction can repair an invalid final outcome up to twice within the same session budget. If a document still fails extraction, hosted generation saves the partial results and failure details, marks the job and Activity run as failed, and skips the automatic baseline run. Retry generation to complete coverage.
...
- **Agent** — the agent's own page: every piece of work it did across the workspace's repositories (a spec scan, a guard setup, a generate), newest first, narrowed by kind, status and repository, each opening as the run's work: its steps as headings, each listing what it did as the engine recorded it (deterministic work, cache hits and session work alike), and under each step one row per piece of work with its status, its kind ("Scenario author", "Fidelity check") and how long it took, fidelity checks nested under the worker that spawned them. A row opens its transcript beside the list, verbatim from the stored journal: the system prompt, every user message (the briefing and the loop's own interventions), every model turn with its tool call and the result that answered it, the outcome value, and the loop's events as their fields. Nothing is paraphrased by the client. A live piece of work tails as it happens; a run that stopped carries its reason under the step it stopped on. The toast a starting run raises and the notifications it files open the run's own conversation. Served by `GET /api/sessions/runs` (workspace-wide, `?repo=&kind=&status=&limit=&before=`), `GET /api/sessions/runs/:runId`, and the per-repository history pages `GET /api/repos/:id/sessions/runs/:command/:runId/activity?after=&limit=`.
...
Connected repositories manage these sites in the dashboard's Sources tab. Hosted fetches accept public HTTP(S) destinations only, validate every redirect, and pin connections to validated DNS addresses. Overlapping source edits return HTTP 409; reload and retry the rejected edit. Removing the last source keeps the Rescan indicator active until a scan absorbs the removal. The Sources viewer reads the latest fetched page through `GET /api/repos/:id/spec/source-doc?ref=...`, so Refresh updates an open page immediately. Corpus views continue to read their stored scan snapshots, including commit-pinned views.
