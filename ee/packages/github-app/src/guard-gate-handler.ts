...
export async function handlePullRequestGuardGate(
  ...
  // A duplicate delivery must REUSE the head's existing queued/in-progress run
  // rather than open a second one: GitHub evaluates the NEWEST run per name+sha,
  // so a newer run would shadow the verdict later posted to the active one.
  const existingCheckRunId = await findActiveCheck(octokit, coords, GUARD_GATE_CHECK_NAME, headSha);
  const checkRunId =
    existingCheckRunId ?? (await startCheck(octokit, coords, GUARD_GATE_CHECK_NAME, headSha));

  let jobId: string | null;
  try {
    jobId = await deps.enqueueGuardGate({
      repoFullName,
    ...
    // A REJECTED enqueue (worker down, DB error) left no live job behind — any
    // tracked row was already marked failed — so nothing, not even boot orphan
    // settlement, would ever complete the Check: settle it as the gate's
    // infra-error failure now (best-effort), then rethrow so the webhook layer
    // logs the enqueue failure. Only a run THIS delivery created, though — a
    // reused run belongs to the gate job already running for this head, and
    // failing it here would clobber the verdict that job is about to post.
    if (checkRunId != null && existingCheckRunId == null) {
      await postGuardGateErrorCheck(octokit, { repoFullName, headSha, checkRunId }).catch(
        () => undefined,
      );
    }
    throw err;
  ...
  // A REUSED run is that job's own — completing it as neutral here would
  // overwrite the verdict the job is about to post, so leave it untouched.
  if (jobId === null && checkRunId != null && existingCheckRunId == null) {
    await postCheck(
      octokit,
      coords,
