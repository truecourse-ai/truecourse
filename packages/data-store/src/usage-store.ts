  ...
  async record(delta: UsageDelta): Promise<string> {
    const [row] = await this.db
      ...
      .values({
        ...
        inputCostUsd: delta.inputCostUsd.toFixed(8),
        outputCostUsd: delta.outputCostUsd.toFixed(8),
        cachedCostUsd: delta.cachedCostUsd.toFixed(8),
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          inputCostUsd: sql`${llmUsage.inputCostUsd} + excluded.input_cost_usd`,
          outputCostUsd: sql`${llmUsage.outputCostUsd} + excluded.output_cost_usd`,
          cachedCostUsd: sql`${llmUsage.cachedCostUsd} + excluded.cached_cost_usd`,
  ...
  async series(query: UsageQuery, bucket: 'day' | 'week'): Promise<UsageSeriesRecord[]> {
    ...
    const rows = await this.db
      .select({
        ...
        costUsd: sql<string>`coalesce(sum(${llmUsage.costUsd}), 0)`,
        inputCostUsd: sql<string>`coalesce(sum(${llmUsage.inputCostUsd}), 0)`,
        outputCostUsd: sql<string>`coalesce(sum(${llmUsage.outputCostUsd}), 0)`,
        cachedCostUsd: sql<string>`coalesce(sum(${llmUsage.cachedCostUsd}), 0)`,
        inputTokens: sql<string>`coalesce(sum(${llmUsage.inputTokens}), 0)`,
        outputTokens: sql<string>`coalesce(sum(${llmUsage.outputTokens}), 0)`,
        cacheReadTokens: sql<string>`coalesce(sum(${llmUsage.cacheReadTokens}), 0)`,
        cacheCreateTokens: sql<string>`coalesce(sum(${llmUsage.cacheCreateTokens}), 0)`,
    ...
    return rows.map((row) => ({
      ...
      jobType: row.jobType,
      costUsd: num(row.costUsd),
      inputCostUsd: num(row.inputCostUsd),
      outputCostUsd: num(row.outputCostUsd),
      cachedCostUsd: num(row.cachedCostUsd),
      inputTokens: num(row.inputTokens),
      outputTokens: num(row.outputTokens),
      cacheReadTokens: num(row.cacheReadTokens),
      cacheCreateTokens: num(row.cacheCreateTokens),
