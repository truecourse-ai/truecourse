 ...
 *
 * An authoring session used to get a place id and an
 ...
 *  3. **The api effects** — the frontend→API join:
 ...
 *     recorded as unjoined, never guessed — the `unknown` rule, and the rule
...
export interface WebPlaceContext {
   ...
   *
   * The api derivation composes a mounted tRPC tree into real
...
interface ApiIndex {
  ...
  /** Dotted procedure → api interface id, for the entries derived from a router tree. */
...
function apiOperations(interfaces: readonly Interface[]): ApiIndex {
  ...
  for (const iface of interfaces) {
    ...
    // The procedure index is the RPC join key: the catalog says which
 ...
 * and — the one-hop wrapper resolution — every http call inside a
...
function join(
  ...
  for (const node of closure) {
    ...
    // The RPC half. A procedure the catalog defines (a mounted tRPC
