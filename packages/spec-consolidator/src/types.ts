...
export const ConflictSchema = z.object({
  ...
  /**
   * Plain-English summary of how the candidates differ — generated by
   * an LLM call after merge, cached per (conflictId, candidateFingerprint).
   * Absent when explanation generation is disabled, fails, or cache
   * lookup is being skipped for a test run. The raw diff path list in
   * the dashboard is still the source of truth for engineers; this is
   * the readable layer non-engineers need to decide.
   */
