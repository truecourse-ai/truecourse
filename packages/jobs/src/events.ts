 ...
 * unchanged across multiple server replicas: every instance LISTENs, and the
...
/**
 * The live-push surface the routes and the runner use. `createJobs` builds an
 * {@link EventHub} by default; a caller with no Postgres to LISTEN on (tests)
 * substitutes its own.
 */
export interface EventBackplane {
  start(): Promise<void>;
  stop(): Promise<void>;
...
export class EventHub implements EventBackplane {
  ...
  private async connect(): Promise<void> {
    ...
    client.on('notification', (msg) => {
      ...
      try {
        ...
        log.warn(`[jobs] dropping malformed event payload: ${(err as Error).message}`);
      }
    });
    client.on('error', (err) => {
      log.warn(`[jobs] event-hub connection error: ${err.message}`);
      this.client = null;
    ...
    this.client = client;
    log.info('[jobs] event hub listening on tc_events');
  }
...
export async function publishEvent(db: Db, orgId: string, event: ServerEvent): Promise<void> {
  ...
  try {
    ...
    log.warn(`[jobs] pg_notify failed: ${(err as Error).message}`);
  }
}
