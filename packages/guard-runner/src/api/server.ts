 ...
 * (through the shared `spawnServiceProcess` — one spawn shape for every managed
 * service, either driver's), {@link awaitApiServerReady} waits for health, and the
 * handle exposes `signal`, `waitForExit` and `exit` — so a scenario's `boot` /
 * `signal` steps drive the very same process the implicit boot does, with no
 * second spawn path to drift from.
 *
...
import { spawnServiceProcess, type ServiceProcessExit, type ServiceProcessHandle } from '../service-process.js'
...
export interface ApiServerHandle extends ServiceProcessHandle {
  port: number
...
export type ApiServerExit = ServiceProcessExit
