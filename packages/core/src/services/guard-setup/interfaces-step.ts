/**
 * THE INTERFACES STEP of `guard setup` (plan 03 steps 11 + 12) — the seam body
 * the command adapter injects as `GuardSetupOptions.authorInterfaces`. Two
 * halves, in order:
 *
 * 1. RECONCILE (step 12). The cli union's tree-vs-probe disputes — and ONLY
 ...
 *
 * 2. AUTHOR (step 11). The web-task authoring run — the existing
...
export function buildInterfacesStep(
  ...
  return async (input) => {
    ...
    // ---- Half 1: reconcile the cli disputes (step 12). ----------------------
    ...
    // ---- Half 2: author the web tasks (step 11). ----------------------------
    ...
    try {
      ...
      // one diagnostics stream (01-D left the CLI/dashboard not printing them;
      // the setup report row is where they land).
