/**
 * INTERFACE RECONCILIATION — `guard-setup.reconcile-interfaces` (plan 03 step
 * 12): the ONE session per setup run that settles what the cli UNION could not.
 *
 ...
 *
 * SANDBOX-ONLY, NO CODE ACCESS (§7.4 decision). The session's one tool is
...
/**
 * The three numbers (§3.3). The job is a handful of `run_entry` observations —
