...
/** The three numbers: a repair is a handful of sandbox probes and one or
      ...
      try {
        ...
        if (verdict.ok) {
          const caveats = verdict.warnings?.length
            ...
            : '';
          return {
            content: `VERIFIED: install, build, the entry probe, the server boot and (when declared) the web boot all passed. Produce this proposal as the outcome.${caveats}`,
          };
        }
...
- \`verify_recipe\` is the REAL verification — install → build → entry probe → services → server boot → web boot, minutes of work. Run it on your complete proposal before you produce the outcome; the engine re-runs exactly it on whatever you return, so an outcome it has not passed is an outcome that will be refused.
