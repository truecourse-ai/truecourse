...
export function coverageLines(web: boolean): string[] {
  return [
    ...
    '- STATE THE APP PRODUCES (the output of a background job, a processed or uploaded file, a generated preview or archive) is produced by the app\'s own code: the seed runs once the services are up and before any server boots, so no server of the app is running and none of its routes can be called. Import and call the module that does the work (the job\'s handler, the worker\'s processor, the service function the route calls) from the seed, and wait for it to finish, then prove it by execution like every fixture. Never by inserting its rows alone. When the app\'s code cannot be driven from the seed (it needs a running server, or a service the recipe does not start), it is unmet.',
