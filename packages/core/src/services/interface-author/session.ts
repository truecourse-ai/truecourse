...
1. **Locators, in this order of preference.** Every step \`target\` is an object naming ONE handle — never a selector string, an XPath or a test id:
   ...
   - \`within\` scopes a step to one container and takes any handle: \`{"role": "dialog", "name": "Delete expense"}\`, \`{"role": "main"}\`, or \`{"css": "main"}\` (non-canonical, so \`why\` is needed).
