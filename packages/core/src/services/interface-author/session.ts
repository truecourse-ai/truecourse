...
- An input or activate step inside a dialog/panel can carry \`within: { "role": "dialog", "name": "Delete expense", "exact": true }\`. Use the actual container to distinguish a confirmation button from the page's identically named opener, or a page control from the sidebar's control of the same name. Do not rely on the first match when only one of them serves the task.
...
1. **Locators, in this order of preference.** Every step \`target\` is an object naming ONE handle — never a selector string, an XPath or a test id:
   ...
   - **Visible text or attribute** — \`{"title": "More"}\`, \`{"label": "Email"}\`, \`{"placeholder": "Search"}\`, \`{"text": "Show all"}\`, \`{"alt": "Logo"}\`: for a control with no role+name that a user can still identify on the screen.
   - **CSS, last** — \`{"css": "main button:has(i.bi-chevron-expand)"}\`: only when neither of the above reaches the control (an icon-only button with no label, two controls sharing one tooltip). A step with \`css\` in its target or its \`within\` is NON-CANONICAL: it carries \`"why"\` on the step — one line saying what the control is and why nothing accessible reaches it ("icon-only button, no aria-label; SortDropdown.tsx renders <i class=bi-chevron-expand>") — it is proven on the live screen by \`check_draft\`, and it is recorded as a non-canonical locator. Within css, prefer a stable attribute (\`title\`, \`data-*\`, \`aria-*\`, the icon's class), then position inside a landmark or named region (\`main …\`, \`nav[aria-label="Sidebar"] …\`), then bare position; NEVER a generated utility class (Tailwind utilities, CSS-module hashes). Copy a selector from the observation's list of unnamed controls rather than composing one. With no live screen, \`css\` is refused: name the control in \`unresolved\` instead.
   ...
   - \`within\` scopes a step to one container and takes any handle, a role always with its name: \`{"role": "dialog", "name": "Delete expense"}\`, \`{"role": "navigation", "name": "Sidebar"}\`, or \`{"css": "main"}\` (non-canonical, so \`why\` is needed).
