...
export function openRoslynHost(): RoslynHostSession {
  ...
  const settleWith = (resp: HostResponse) => {
    ...
    // A response with nothing waiting for it means the host and this client
    // disagree about the protocol — louder than silently dropping it.
    if (!next) return
...
/** Open a session, issue one request, and shut the host down again. */
