 ...
 * restates a well-known constant in a way readers must decode. The
...
export const csharpPreferUnixEpochFieldVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode) {
    ...
    return makeViolation(
      ...
      `\`new ${name}(1970, 1, 1, …)\` reconstructs the Unix epoch by hand; \`${name}.UnixEpoch\` names the same constant directly.`,
      sourceCode,
