 ...
 * delegate type per event. The check targets an `event_field_declaration`
...
export const csharpNonGenericEventHandlerVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode) {
    ...
    return makeViolation(
      ...
      `Event uses the custom non-generic delegate \`${name}\` where \`EventHandler<TEventArgs>\` would provide a strongly-typed \`(sender, e)\` signature without a bespoke delegate type.`,
      sourceCode,
