 ...
 * graph and can be deleted (S1185/RCS1132). The forwarded call must target the
...
function report(ruleKey: string, node: SyntaxNode, filePath: string, sourceCode: string, name: string) {
  return makeViolation(
    ...
    'Redundant override',
    `\`${name}\` overrides the base member only to forward to it with the same arguments — the override adds nothing and can be removed (S1185/RCS1132).`,
    sourceCode,
