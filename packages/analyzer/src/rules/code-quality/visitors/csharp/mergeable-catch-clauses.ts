 ...
 * removing the maintenance hazard of two bodies drifting apart (S2327). The
    ...
    for (const clause of catches) {
      ...
      if (dup) {
        return makeViolation(
          ...
          'This catch clause has the same handler body as an earlier catch on the same `try`; combine them into one catch (S2327).',
          sourceCode,
