 ...
 * (S6670). Matched on the `Trace.Write`/`Trace.WriteLine` member-access shape;
...
export const csharpTraceWriteUsageVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode) {
    ...
    return makeViolation(
      ...
      `\`Trace.${method}\` routes application logging through \`System.Diagnostics.Trace\`, which has no levels or structure — use a logging abstraction such as \`ILogger\` instead (S6670).`,
      sourceCode,
