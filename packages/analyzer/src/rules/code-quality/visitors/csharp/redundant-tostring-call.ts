 ...
 * interpolation hole — is redundant work the compiler already does.
...
function report(ruleKey: string, node: SyntaxNode, filePath: string, sourceCode: string, where: string) {
  return makeViolation(
    ...
    `\`.ToString()\` is called on a value already in ${where}, where it is converted to a string anyway — the call is redundant.`,
    sourceCode,
