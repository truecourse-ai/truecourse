...
/**
 * True when `node` sits inside an EF Core migration — a class whose base list
 * names `Migration` (`Microsoft.EntityFrameworkCore.Migrations.Migration`).
 * These files are scaffolded by `dotnet ef migrations add`, run once, and
 * regenerated wholesale on the next schema change, so their (often very long)
 * `Up`/`Down` bodies are generated artifacts, not hand-maintained code — the
 * same category as `<auto-generated>` output for maintainability rules.
 */
export function isCSharpEfMigration(node: SyntaxNode): boolean {
  const cls = getCSharpEnclosingClass(node)
  if (!cls) return false
  ...
  return baseList.namedChildren.some((base) => {
    ...
    // A base entry is an identifier / generic_name / qualified_name; take its
    // simple (rightmost) name so `Migrations.Migration` matches `Migration`.
