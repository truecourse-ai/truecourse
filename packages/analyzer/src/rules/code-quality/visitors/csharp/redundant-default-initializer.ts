 ...
 * guarantees for unassigned fields; the initializer is noise (CA1805).
  ...
  visit(node, filePath, sourceCode) {
    ...
    for (const declarator of varDecl.namedChildren) {
      ...
      return makeViolation(
        ...
        `Field \`${name}\` is explicitly initialized to its type's default — the runtime already guarantees this, so the initializer is redundant (CA1805).`,
        sourceCode,
