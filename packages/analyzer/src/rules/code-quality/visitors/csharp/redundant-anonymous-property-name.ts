 ...
 * is redundant and can be dropped to `new { x.Name }`. Detected by an
...
function report(ruleKey: string, node: SyntaxNode, filePath: string, sourceCode: string) {
  return makeViolation(
    ...
    `\`${node.text} = …${node.text}\` repeats a name the compiler already infers from the source member — drop the explicit \`${node.text} =\`.`,
    sourceCode,
