 ...
 * infer or spell out all of them, a brain-overload signal (S2436). Scoped to
...
export const csharpTooManyGenericParametersVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode) {
    ...
    return makeViolation(
      ...
      `Method \`${name}\` declares ${count} generic type parameters (> ${MAX_TYPE_PARAMETERS}), which callers must infer or spell out at every call (S2436).`,
      sourceCode,
