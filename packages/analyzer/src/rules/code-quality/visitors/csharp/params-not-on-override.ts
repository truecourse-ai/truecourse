 ...
 * still spread arguments. The check fires on an `override` method whose
...
export const csharpParamsNotOnOverrideVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode) {
    ...
    return makeViolation(
      ...
      `Override \`${methodName}\` declares its final array parameter \`${paramName}\` without \`params\`; if the base method is variadic, callers reaching it through this type lose the variadic call form.`,
      sourceCode,
