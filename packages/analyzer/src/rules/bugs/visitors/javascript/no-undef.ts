...
export const noUndefVisitor: CodeRuleVisitor = {
  ...
  visit(node, filePath, sourceCode, dataFlow?: DataFlowContext) {
    ...
    // Skip service-worker / Mock Service Worker scripts. These run
    // in ServiceWorkerGlobalScope where \`addEventListener\`,
    // \`skipWaiting\`, \`clients\`, etc. are real globals.
    const basename = filePath.split('/').pop()?.toLowerCase() ?? ''
    if (basename === 'mockserviceworker.js' ||
        basename === 'sw.js' || basename === 'service-worker.js' ||
        /\.sw\.js$/.test(basename)) return null
    // Detect by header comment — MSW writes "Mock Service Worker"
    // / "Generated by Mock Service Worker" near the top.
    if (/Mock Service Worker|self\.skipWaiting\(|self\.clients\b/.test(sourceCode.slice(0, 4000))) return null
