...
/**
 * Recognize machine-generated source files (parser/lexer scaffolding, codegen
 * output) by their conventional banner comments and path markers. Rules that
 * flag stylistic / structural patterns produce noise on these files since the
 * code shape is dictated by the generator, not the author. The first ~10 lines
 * are enough — every common generator stamps a banner at the top of the file.
 */
const GENERATED_BANNER_PATTERNS = [
  /\bGenerated from\b/i,
  ...
  /@generated\b/,
  /\beslint-disable\b.*\bgenerated\b/i,
]
const GENERATED_PATH_PATTERN = /\.(gen|generated)\.[^/]+$|(?:^|\/)generated\/|(?:^|\/)__generated__\//

export function isGeneratedFile(filePath: string | undefined, sourceCode: string): boolean {
  if (filePath && GENERATED_PATH_PATTERN.test(filePath)) return true
  const head = sourceCode.slice(0, 800)
  for (const pat of GENERATED_BANNER_PATTERNS) {
