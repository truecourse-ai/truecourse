...
- `specs/` — the spec-consolidation store for `truecourse spec scan`:
  ...
  - `specs/decisions.json` — **committable**, user-authored. Curated resolutions: doc→doc `relations[]` (replace/precedence/keep-both), `manualAreas[]` (area-tag overrides), and `manualIncludes[]` (relevance-filter overrides).
