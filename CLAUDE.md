...
- `packages/shared/` — Shared Zod schemas and TypeScript types. Also home of the agentic loop's driver-agnostic pieces (`src/llm/`): the session contract (events, defs, driver seam, store shapes) and the policy shell `runAgentLoop` (`agent-loop.ts` — budgets, ceilings, resume grants, malformed policy, seq/ts stamping, depth-1 children; no `ai`/SDK/fs imports)
