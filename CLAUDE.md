...
- `guard/` — the guard run store for `truecourse guard run`, mirroring the analyze store (there is **no** `diff.json` — guard shows current state only). See `packages/guard-runner/src/store.ts`; the committed scenarios it runs live in `scenarios/` (`recipe.json`, `manifest.json`, `<area>/*.yaml`, and `decisions.json` — the user-authored decisions `guard generate` honors (`dismissedClaims`, `dismissedFlows`, and `reenabledFlows` — the manual reset for a retired flow), the spec `decisions.json` analog — all committable).
  ...
  - `guard/authoring/<runId>/<flowId>.<surface>.jsonl` — **gitignored** append-only worker-session transcripts of one `guard generate` run (the live feed the dashboard tails; replayable afterwards). See `packages/guard-runner/src/store.ts`.
  - `guard/auto-resolutions.json` — **gitignored** durable auto-resolve ledger for `guard generate`: per-flow counts of auto-resolved "the test is wrong" verdicts and exhausted authoring worker sessions (sources `fidelity` / `author`; `triage` only in ledgers written by the pre-worker pipeline), with their verdict history, the tainted flows whose next generate bypasses the author cache, and the RETIRED flows — past the threshold a flow retires (a quiet `retired` coverage gap, zero further spend, never a human task) until one of three resets fires: the bound spec content moves, the authoring prompt fingerprint moves, or a `reenabledFlows` entry in `scenarios/decisions.json`. Transient run memory; safe to delete (auto behaviors just start their budget over).
  ...
  - `guard/result.json` — **gitignored** run-result of the last `guard generate` (written/settled/punt/birth-finding/error counts, per-section gap reasons, the worker-reported `journeyDefects`, the `authoringRunId` naming the run's `guard/authoring/` transcripts, call+token+cost totals, and the per-stage LLM-failure tallies — a stage that lost EVERY call aborts the run as `status: 'llm-failed'` and rewrites nothing else). The CLI `guard status` and the dashboard coverage view render the same summary from it.
