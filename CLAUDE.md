...
- `guard/` — the guard run store for `truecourse guard run`, mirroring the analyze store (there is **no** `diff.json` — guard shows current state only). See `packages/guard-runner/src/store.ts`; the committed scenarios it runs live in `scenarios/` (`recipe.json`, `manifest.json`, `flows.json`, `claims.json`, `<area>/*.yaml`, and `decisions.json` — the user-authored `dismissedClaims` `guard generate` honors, the spec `decisions.json` analog — all committable).
  - `scenarios/claims.json` — **committable**. The extracted claim corpus: the unit of meaning and the denominator of claim-keyed coverage. Per claim `id` (the handle a scenario step's `milestone` tag references), identity `doc` + `anchor` + `title` (the same triple `decisions.json` dismisses on), the verbatim claim sentence, `verifyVia`, `needs[]`, `notes`, and a `contentHash` over the claim's OWN doc/anchor/title/sentence — never the section fingerprint, so a doc edit invalidates exactly the claims whose content changed. Also carries `untestable[]`: the doc statements extraction consciously refused, with reasons. Committable because the scenarios and flows that reference it are; `loadScenarios` resolves every reference against it and reports a dangling one as a load error (`packages/guard-runner/src/claim-refs.ts`). Schema in `packages/shared/src/guard/claims.ts`.
