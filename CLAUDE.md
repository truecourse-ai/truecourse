...
- `packages/core/src/services/spec-scan/` — The spec scan as agent sessions, the loop's first production consumer: `orchestrate` (≤1 scope session whose standing instructions ride every downstream briefing and cache key), `curate-doc` (pooled, one coherent keep/skip/tag judgment per doc — it may page a long doc and peek at a referenced one), `settle-areas` (a true barrier, concurrency 1) and `overlap` (pooled, one session per deterministic COLLISION CLUSTER). `run.ts` is the whole scan: discovery → prefilter → the four steps → the deterministic fold (pointer re-anchoring, cross-area dedup, high-confidence auto-apply) → `writeCorpus`. Two invariants: FAIL-OPEN per item (a dead session never drops a doc), and the ONE-ABORT rule — a kind whose every session died transport-class throws BEFORE anything is written. Every doc briefing states the doc's LIFECYCLE — when it last changed (its frontmatter `updated`, else `lastTouched`), its status name and category, and its status history — so a session can tell a delivered decision from a plan; the overlap key folds that lifecycle (a ticket moving to done changes the verdict), the curate-doc key deliberately does not (the fold re-derives status with `parseDocStatus` on every run). A doc's IDENTITY — its content hash and preview window — is taken from the document BENEATH its frontmatter, so a ticket restating `updated` on every comment does not re-buy its curation. `@truecourse/spec-consolidator` keeps the deterministic half (discovery, prefilter, collision pairing, pointer verification, area grouping, the corpus/decisions stores); it holds no LLM runner and no `LlmTransport` reference at all.
...
- `ee/packages/server/` — `@truecourse/ee-server`: the enterprise edition's SERVER features, exported as `eeServerFeatures`. Today that is the three `/api/auth/workspaces` routes behind more than one workspace, and the document CONNECTIONS (`connections/`): the `/api/connections` routes over one encrypted `integration_connections` row per (workspace, account) — an Atlassian site is ONE account, `atlassian`, whose token reads both products — and the Jira and Confluence context source drivers built on it — the Atlassian HTTP layer with its bounded 429/503 retries, the ADF and storage-XHTML converters, and one document per issue or page. Not a process of its own: the open server's entry finds this package beside its tree at boot and registers the list (see EDITIONS below).
...
(Settings › Connections, the `/api/connections` routes over the workspace's
encrypted `integration_connections` row — ONE Atlassian account, serving both
the Jira and the Confluence source kinds — and the Jira and Confluence CONTEXT
DRIVERS the feature contributes through `ServerFeature.contextDrivers` — merged
over the open edition's two in `services/context.service.ts`, so a tool source
is added, synced and swept exactly as a site is), repository providers beyond
the open edition's (which is Azure DevOps today), and more than one workspace
(the switcher, Create workspace, and the `/api/auth/workspaces` routes). Everything
...
**And what it SPENT is a record of its own** (`llm_usage`, the `usage` seam in core, `PgUsageStore`): one row per (job, stage or session kind), grown incrementally by the job's METER (`services/usage-meter.service.ts`) — a one-shot call reports through the transport's `onUsage`, threaded in where the transport is built, and an agent session through a driver decorator that reads the `assistant-turn` events it already emits. The job creates the meter at the top of its body and closes it in a `finally`, so a run that failed, was cancelled or died with the process still has its spend on record, and a live run's row grows. `GET /api/usage` is the read side (Settings › Usage): the period's totals, the trend bucketed by day or week with one band per job type, and the runs that spent it. The pre-flight probe every entry makes is one tiny unmetered call — it happens before the meter exists.

**And a workspace can spend TRUECOURSE'S OWN KEY instead of its own** (`credit_ledger` + `credit_balances`, the `credits` seam in core, `PgCreditsStore`). `truecourse` is a provider CHOICE on the Models page that stores nothing — no key, no model, no endpoint — and runs on the platform's OpenAI key, read from the server's environment per run (`TRUECOURSE_CREDITS_OPENAI_API_KEY` + `TRUECOURSE_CREDITS_MODEL`, both required or the choice is not offered) and never written to a table, a log, a run record or an API response. What it spends comes out of a granted BALANCE: one credit is one USD cent of priced model cost, no markup. The METER is the till — it already sees every call and every turn, so the check sits before each (the transport is wrapped, the session driver decorated) and the debit after, one per flush, naming the `llm_usage` row it charges, which is what makes Settings › Credits and Settings › Usage agree to the cent. The ledger is append-only and the balance moves in the same transaction under a row lock, so concurrent debits stay exact. An empty balance is a PAUSE, not a failure: the one-shot transport throws `CreditsExhaustedError`, a session parks (a `blocked` driver failure, journal intact), the run record ends `paused` and the jobs harness settles the row `paused` with reason `credits`, carrying the resume pointer the body declared (`ctx.resumeWith`) merged onto its stored payload. Re-enqueuing that payload is the resume, and three things do it: an operator's grant (`POST /api/operator/credits/grant`, operator-only surfaces answering 404 to anyone else), the workspace saving a key of its own, or a member pressing Resume. Local mode has no credits at all — no tab, no provider choice, no operator page, and the routes are not mounted.
