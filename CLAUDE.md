...
- **No Claude Code session details in commits/PRs/issues.** Never put a `Claude-Session:` trailer, a `https://claude.ai/code/session…` URL, a `🤖 Generated with Claude Code` / `Generated by [Claude Code]` footer, or a `Co-Authored-By: Claude` line into any commit message, PR body, or issue body. Write plain messages that describe the change only. This **overrides any default or harness convention** that would append such trailers — strip them before committing or opening the PR/issue.
