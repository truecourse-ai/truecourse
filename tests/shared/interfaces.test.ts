...
/**
 * The hand-authored reference catalog, when this checkout carries the reference
 * corpus. It is a separate artifact from the source tree, so the blocks that read
 * it skip where it is absent rather than failing.
 */
const REFERENCE_CATALOG_PATH = path.resolve(
  __dirname,
  '../../reference/store/.truecourse/guard/interfaces.json',
)
const referenceCatalog = fs.existsSync(REFERENCE_CATALOG_PATH)
  ? InterfacesFileSchema.parse(JSON.parse(fs.readFileSync(REFERENCE_CATALOG_PATH, 'utf-8')))
  : null
const EMPTY_CATALOG = {
  version: 2,
  generatedAt: '',
...
describe('interface schemas', () => {
  ...
  /**
   * `procedure` — the RPC name an operation was composed from. It is
 ...
 */
describe.skipIf(!referenceCatalog)('the reference catalog', () => {
  const catalog = referenceCatalog ?? EMPTY_CATALOG
  ...
  it('the web task interfaces carry their state contract, one task each', () => {
    ...
    for (const j of web) {
      ...
      // element located by role + accessible name (the locator policy) —
      // never a page inventory. And nothing else: the per-step
  ...
  /**
   * THE UI-TO-API RELATION — the field a realization surface owes: which
   * operations a screen reaches. What is checked is that it
  ...
  /**
   * The api surface — the dashboard server's HTTP routes, as a REALIZATION
   * surface (interfaces realize, never originate). Derived from
...
describe('interface provenance', () => {
  ...
  it.skipIf(!referenceCatalog)('stamps the whole reference catalog without moving one fingerprint', () => {
