...
describe('0021_drop_ee_era', () => {
  it('keeps every document connection with its token; the cached sweep delta is cleared, then dropped by 0028', async () => {
    ...
    // 0021 nulled the column; 0028_integration_connections removed it.
    const columns = await sql.query<{ column_name: string }>(
