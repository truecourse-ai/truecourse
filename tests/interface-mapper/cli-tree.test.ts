/**
 * The primary cli derivation: analyzer `cliCommands` artifacts → interfaces. The
...
describe('deriveCliInterfacesFromTree', () => {
  const interfaces = deriveCliInterfacesFromTree([analyze('src/cli.ts', CLI_SOURCE)])

  it('emits one interface per command path, id-slugged and sorted', () => {
  ...
  it('roots each interface at its argv path with a single invoke step', () => {
    const configSet = interfaces.find((j) => j.id === 'cli/config-set')
  ...
  it('carries the command description as a cosmetic step label', () => {
    const deploy = interfaces.find((j) => j.id === 'cli/deploy')
  ...
  it('maps nothing for a repo with no cli surface', () => {
    ...
    expect(deriveCliInterfacesFromTree([service])).toEqual([])
  })

  it('collapses a command declared in two files onto one interface, flags unioned', () => {
    const base = analyze(
      'src/cli.ts',
      `
      import { Command } from 'commander'
...
describe('interface fingerprints — surface-visible shape only', () => {
  it('survives a file rename, a handler rename, and a description rewrite', () => {
    const original = deriveCliInterfacesFromTree([analyze('src/cli.ts', CLI_SOURCE)])
    const refactored = deriveCliInterfacesFromTree([
      analyze(
  ...
  it('moves when a command gains a flag', () => {
    const before = deriveCliInterfacesFromTree([analyze('src/cli.ts', CLI_SOURCE)])
    const after = deriveCliInterfacesFromTree([
      analyze(
        'src/cli.ts',
        CLI_SOURCE.replace(
          `.option('--json', 'Emit machine-readable JSON')`,
  ...
  it('ignores the order flags are declared in', () => {
    ...
    const before = deriveCliInterfacesFromTree([analyze('src/cli.ts', CLI_SOURCE)])
    const after = deriveCliInterfacesFromTree([analyze('src/cli.ts', swapped)])
