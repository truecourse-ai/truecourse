...
/**
 * The Interfaces tab's MAP action (OSS): `POST /:id/guard/map` derives the
 * interface catalog from the working tree and answers with the fresh view.
 *
...
describe('Guard map action', () => {
  ...
  it('writes the catalog snapshot and answers with the interfaces view', async () => {
    ...
    expect(res.body.interfaces.map((i: { id: string }) => i.id).sort()).toEqual(['cli/add', 'cli/list']);
    expect(res.body.totals).toMatchObject({ interfaces: 2, grounded: 0, ungrounded: 2 });
    ...
    expect(JSON.parse(fs.readFileSync(snapshot, 'utf-8'))).toMatchObject({ version: 2 });
