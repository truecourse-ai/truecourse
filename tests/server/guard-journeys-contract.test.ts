 ...
 * /guard/journeys` must carry each journey's full grammar and its io facts
 ...
 *  - a journey WITH one arrives byte-identical, `unknown` exit statuses included.
 */
...
describe('Guard journeys — the contract passthrough', () => {
  ...
  it('keeps `unknown` and "established as none" intact across the wire', async () => {
    const res = await request(app).get(url('journeys')).expect(200);
    const io = res.body.journeys.find((j: { id: string }) => j.id === 'cli/tasks-add').contract.commands[0].io;
