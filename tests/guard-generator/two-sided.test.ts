...
describe('two-sided promises — the negative half SURVIVES the pipeline (G15 regression)', () => {
  ...
  it('a scenario realizing one milestone with BOTH halves commits with both steps intact', async () => {
    ...
    // milestone) passes the worker's settle gate, confirms green against the
    // real fixture, and lands in the corpus with the rejection step INTACT.
    const r = repo()
    writeRecipe(r)
    ...
    const res = await runGenerate({
      ...
      turnFn: workerTurnBy({ strictness: twoSided }),
    })
