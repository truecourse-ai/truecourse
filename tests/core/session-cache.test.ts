...
import { z } from 'zod'
import { getCacheEntry, setCacheEntry, setKvCacheStore } from '@truecourse/llm'
...
/** The stored entry, straight off the seam the cache writes through. */
const entry = (): Promise<unknown | null> => getCacheEntry(repo, CACHE_NAME, KEY)
...
describe('cachedSessionOutcome', () => {
  ...
  it('treats an entry the schema rejects as a miss and overwrites it', async () => {
    await setCacheEntry(repo, CACHE_NAME, KEY, { tasks: 'not an array' })
    ...
    expect(await entry()).toEqual({ tasks: ['web/settings'] })
  })
