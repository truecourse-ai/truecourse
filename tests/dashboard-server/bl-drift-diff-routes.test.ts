...
/** The Contracts + Spec PR diffs (EE per-commit) — head at `?ref` vs the verify baseline. */
...
describe('BL-Drift PR diff routes', () => {
  ...
  it('contracts/diff: added / removed / modified by path + content', async () => {
    ...
    // A real contract change means the head regenerated the spec → claims at the head.
    await saveSpec({ repoKey, commitSha: 'head' }, 'claims', {
      generatedAt: '2026-01-02T00:00:00.000Z',
      modules: [],
      claims: [{ module: 'orders', topic: 'x', subject: 'y' }],
    });
  ...
  it('spec/diff: added/removed claims + newConflictCount of head conflicts not in base', async () => {
    const repoKey = fixture.repoPath;
    const claim = (module: string, topic: string, subject: string) => ({ module, topic, subject });
    await saveSpec({ repoKey, commitSha: 'base' }, 'claims', {
      generatedAt: '2026-01-01T00:00:00.000Z',
      modules: [],
      claims: [claim('auth', 'login', 'kept'), claim('auth', 'login', 'gone')],
    });
    await saveSpec({ repoKey, commitSha: 'head' }, 'claims', {
      generatedAt: '2026-01-02T00:00:00.000Z',
      modules: [],
      claims: [claim('auth', 'login', 'kept'), claim('billing', 'tax', 'new')],
    });
    await saveSpec({ repoKey, commitSha: 'base' }, 'scanState', { openConflicts: [{ id: 'c-old', topic: 'auth', subject: 'old' }] });
    await saveSpec({ repoKey, commitSha: 'head' }, 'scanState', {
      openConflicts: [
        { id: 'c-old', topic: 'auth', subject: 'old' },
        { id: 'c-new', topic: 'billing', subject: 'introduced' },
      ],
    });
    await writeVerifyLatest(repoKey, baseline('base'));

    const res = await request(app)
      .get(`/api/repos/${fixture.project.slug}/spec/diff?ref=head`)
      .expect(200);
    ...
    expect(res.body.openConflicts).toHaveLength(2);
    expect(res.body.newConflictCount).toBe(1); // only c-new is introduced by the PR
  });

  it('code-only PR head (nothing stored at the ref) ⇒ empty diff, NOT all-removed', async () => {
    const repoKey = fixture.repoPath;
    // The baseline has contracts + claims, but the PR head stored nothing (the gate
    // reuses the base spec/contracts for a code-only PR). The diff must be empty.
    await seedContracts(repoKey, 'base', { 'orders/keep.tc': 'entity Keep {}' });
    await saveSpec({ repoKey, commitSha: 'base' }, 'claims', {
      generatedAt: '2026-01-01T00:00:00.000Z',
      modules: [],
      claims: [{ module: 'auth', topic: 'login', subject: 'kept' }],
    });
    await writeVerifyLatest(repoKey, baseline('base'));
    ...
    const s = await request(app)
      .get(`/api/repos/${fixture.project.slug}/spec/diff?ref=headonly`)
      .expect(200);
    expect(s.body.added).toEqual([]);
    expect(s.body.removed).toEqual([]);
  });

  it('PARTIAL head contracts (reapplied promotion, no claims at head) ⇒ empty diff', async () => {
    const repoKey = fixture.repoPath;
    // Baseline has 3 contracts; the PR head has only 1 (a reapplied promotion) and NO
    // claims artifact (code-only PR). Must NOT report the other 2 as removed.
    await seedContracts(repoKey, 'base', {
  ...
  it('returns empty diffs with no ref', async () => {
    ...
    await request(app).get(`/api/repos/${fixture.project.slug}/spec/diff`).expect(200)
      .then((r) => expect(r.body).toEqual({ added: [], removed: [], openConflicts: [], newConflictCount: 0 }));
  });
});
