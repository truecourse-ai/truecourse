...
describe('GET /api/capabilities', () => {
  ...
  it('returns enterprise + the loaded plugin capabilities when enabled', async () => {
    setEnterpriseEnv();
    await loadEnterprise(); // dynamically imports @truecourse/ee-server
    const app = createApp({ serveStatic: false });
    const res = await request(app).get('/api/capabilities');

    expect(res.status).toBe(200);
    expect(res.body.edition).toBe('enterprise');
    // ee-server plugin declares these capabilities.
    expect(res.body.capabilities).toEqual(
      expect.arrayContaining(['sso', 'workspace']),
    );
  });
