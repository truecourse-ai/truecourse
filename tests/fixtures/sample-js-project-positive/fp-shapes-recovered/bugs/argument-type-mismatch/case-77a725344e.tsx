...
export const WebhookLogsSheet = createCallable<WebhookLogsSheetProps, string | null>(({ call, webhookCall }) => {
  const { t } = useLingui();
  const { toast } = useToast();

  const [activeTab, setActiveTab] = useState<'request' | 'response'>('request');

  const { mutateAsync: resendWebhookCall, isPending: isResending } = trpc.webhook.calls.resend.useMutation({
    onSuccess: () => {
    ...
    <Sheet open={true} onOpenChange={(value) => (!value ? call.end(null) : null)}>
      <SheetContent position="right" size="lg" className="max-w-2xl overflow-y-auto">
        <SheetTitle>
          ...
          <p className="font-mono text-muted-foreground text-xs">{webhookCall.id}</p>
        </SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Payload Tabs */}
          <div className="py-6">
            <div className="mb-4 flex items-center gap-4 border-border border-b">
              <button
            ...
            <div className="group relative">
              ...
              <pre className="overflow-x-auto rounded-lg border border-border bg-muted/50 p-4 font-mono text-foreground text-xs leading-relaxed">
            ...
            {activeTab === 'response' && (
              <div className="mt-6">
                ...
                <div className="overflow-hidden rounded-lg border border-border">
                  <table className="w-full text-left text-sm">
                    <tbody className="divide-y divide-border bg-muted/30">
                      {Object.entries(webhookCall.responseHeaders as Record<string, string>).map(([key, value]) => (
                        <tr key={key}>
                          <td className="w-1/3 border-border border-r px-4 py-2 font-mono text-muted-foreground text-xs">
                            {key}
                          </td>
                          <td className="break-all px-4 py-2 font-mono text-foreground text-xs">{value as string}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
