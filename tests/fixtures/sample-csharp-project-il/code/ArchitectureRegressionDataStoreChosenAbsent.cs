...
// The codebase (SampleApi.csproj) has Npgsql (postgres signal) but no DynamoDB
// package — so `dynamodb` is never observed while `postgres` is.
//
// Expected drifts:
...
// Documentation-only marker type (mirrors the Python doc-only module).
internal static class ArchitectureRegressionDataStoreChosenAbsent
{
}
