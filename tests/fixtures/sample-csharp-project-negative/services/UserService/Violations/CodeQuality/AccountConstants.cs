...
internal class AccountDefaults
{
    ...
    /// <summary>The id of the plan new accounts start on.</summary>
    public static string DefaultPlan() => new PlanCatalog().DefaultPlanId;

    // A private nested catalog: nothing outside AccountDefaults can reach PlanCatalog,
    // so a `public` member on it is dead — exactly what member-more-visible flags.
