...
const ROOT = new URL('../fixtures/sample-project', import.meta.url).pathname;
...
async function buildFlows() {
  ...
  for (const mod of split.modules) fileToModule.set(mod.filePath, `${mod.serviceName}::${mod.name}`);
  ...
  for (const sDep of split.dependencies) {
    ...
    for (const call of sDep.httpCalls) {
      ...
      crossServiceCalls.push({ sourceService, sourceModule, sourceMethod, httpMethod: call.method, url: normalizedUrl, targetService: sDep.target });
    }
  }
  ...
  for (const mod of split.modules) { fileToService.set(mod.filePath, mod.serviceName); fileToModuleName.set(mod.filePath, mod.name); }
  for (const ld of split.layerDetails) { for (const fp of ld.filePaths) { if (!fileToService.has(fp)) fileToService.set(fp, ld.serviceName); } }
  ...
  for (const fa of analyses) {
    ...
    for (const route of fa.routeRegistrations) {
        ...
        if (spec) {
          for (const tf of analyses) {
            if (tf.functions.some(f => f.name === spec.name) || tf.exports.some(e => e.name === spec.name)) {
              moduleName = fileToModuleName.get(tf.filePath) || moduleName; break;
            }
          }
        }
        // Class method: handler is a method on an imported class
      ...
      routeHandlers.set(`${svc}::${route.httpMethod}::${fullPath}`, { handlerName: route.handlerName, moduleName });
    }
  }
  ...
  const graph = new AnalysisGraph({
    ...
    databaseConnections: split.databaseResult.connections.map(c => ({ serviceName: c.serviceName, databaseName: c.databaseName, databaseType: dbTypeMap.get(c.databaseName) || 'unknown' })),
...
describe('TS fixture flow tracing', () => {
  it('produces 4 flows', async () => {
    ...
    console.log('\nMounts:');
    for (const fa of (await Promise.all((discoverFiles(ROOT)).map(f => analyzeFile(f)))).filter(Boolean) as FileAnalysis[]) {
      if (fa.routerMounts?.length) {
        const short = fa.filePath.split('sample-project/')[1];
        for (const m of fa.routerMounts) console.log(`  ${short}: ${m.path} → ${m.routerName}`);
      }
    }
    ...
    for (const f of flows) {
      console.log(`  ${f.name} (${f.trigger}, ${f.steps.length} steps)`);
      for (const s of f.steps) console.log(`    ${s.stepOrder}. ${s.sourceService}/${s.sourceMethod} → ${s.targetService}/${s.targetMethod} [${s.stepType}]`);
    }

    expect(flows.length).toBe(4);
    expect(flows.every(f => f.entryService === 'api-gateway')).toBe(true);
  });
});
