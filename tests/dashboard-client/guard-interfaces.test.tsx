...
describe('Interfaces tab — the recipe as a destination', () => {
  beforeEach(() => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async (url: string | URL) =>
        String(url).includes('/guard/interfaces') ? json(CLI_AND_WEB) : json({}),
      ),
    );
  });
  ...
  it('opens the preparation from a surface row', async () => {
    const user = userEvent.setup();
    renderTab();
    await screen.findByRole('list', { name: 'Interface catalog' });
  ...
  it('offers no raw reading — the stored file has no route to show it', async () => {
    const user = userEvent.setup();
    renderTab();
    await screen.findByRole('list', { name: 'Interface catalog' });
    await user.click(opener('CLI'));
    const recipe = await screen.findByRole('region', { name: 'Recipe' });
    expect(within(recipe).queryByRole('group', { name: 'View mode' })).toBeNull();
  });
