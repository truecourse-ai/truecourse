...
/**
 * A journey carrying the FULL contract — a two-command tree, io as structured
 ...
 */
const WITH_READS: GuardJourneysView = {
  ...MAPPED,
  journeys: [
    {
      ...MAPPED.journeys[0],
...
const WITH_CONTRACT: GuardJourneysView = {
  ...MAPPED,
  journeys: [
    { ...MAPPED.journeys[0], contract: CONTRACT },
...
describe('Journeys tab — the detected-surface banner', () => {
  it('keeps a detected surface that is still awaiting its driver, and drops the undetected ones', () => {
    ...
    // Its journey count lives on the catalog's own surface group beside it.
...
describe('Journeys tab — the mapped catalog', () => {
  ...
  // A journey's truth is its entry in guard/journeys.json, so the detail offers
  // the SAME two readings every artifact-backed entity does — and no third.
  it('switches between the page and the stored journey entry, defaulting to the page', async () => {
    const user = userEvent.setup();
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string | URL) => {
        const u = String(url);
        if (u.includes('/guard/journey/raw')) return json({ id: 'cli/tasks-add', file: 'f.json', content: RAW });
        return u.includes('/guard/journeys') ? json(MAPPED) : json({});
      }),
    );
    renderTab('/repos/r?tab=journeys&gjourney=cli%2Ftasks-add');
    await screen.findByRole('group', { name: 'Journey cli/tasks-add' });
    ...
    expect(screen.queryByLabelText('journey source')).not.toBeInTheDocument();

    await user.click(within(modes).getByRole('button', { name: 'JSON' }));
    await waitFor(() => expect(screen.getByLabelText('journey source')).toHaveTextContent('sha256:j1'));
    // The stored file REPLACES the page — never two readings at once.
    expect(screen.queryByRole('group', { name: 'Journey cli/tasks-add' })).not.toBeInTheDocument();
    expect(screen.queryByText('Used by flows')).not.toBeInTheDocument();

    await user.click(within(modes).getByRole('button', { name: 'View' }));
    expect(await screen.findByRole('group', { name: 'Journey cli/tasks-add' })).toBeInTheDocument();
  });
 ...
 * nothing here repeats the journey name, its entry or its steps. The two honesty
...
describe('Journeys tab — the contract', () => {
  ...
  it('renders no Row shapes block for a command whose listing shape was never established', async () => {
    const command = WITH_CONTRACT.journeys[0].contract!.commands[0];
    const noRows = {
      ...WITH_CONTRACT,
      journeys: [
        {
          ...WITH_CONTRACT.journeys[0],
          contract: {
            ...WITH_CONTRACT.journeys[0].contract!,
            commands: [
              { ...command, io: { ...command.io, produces: { output: command.io!.produces!.output } } },
              WITH_CONTRACT.journeys[0].contract!.commands[1],
            ],
          },
        },
        WITH_CONTRACT.journeys[1],
      ],
    };
    renderPane(noRows, '/repos/r?tab=journeys&gjourney=cli%2Ftasks-add');
  ...
  it('renders the reads as path · when — the seeding side of the file contract', async () => {
    renderPane(WITH_READS, '/repos/r?tab=journeys&gjourney=cli%2Ftasks-add');
    expect(await screen.findByText('Input and output')).toBeInTheDocument();
  ...
  it('says "none" for a command established as reading nothing', async () => {
    renderPane(WITH_READS, '/repos/r?tab=journeys&gjourney=cli%2Ftasks-add&gcmd=tasks+purge');
  ...
  it('prints the command path only where it names one command of a TREE', async () => {
    // A one-command journey: its path IS the journey title the reader just
    // passed, so the contract adds no occurrence of it anywhere on the page.
    const oneCommand: GuardJourneysView = {
      ...MAPPED,
      journeys: [
        { ...MAPPED.journeys[0], contract: { ...CONTRACT, commands: [CONTRACT.commands[0]] } },
        MAPPED.journeys[1],
      ],
    };
    renderPane(oneCommand, '/repos/r?tab=journeys&gjourney=cli%2Ftasks-add');
    expect(await screen.findByText('Grammar')).toBeInTheDocument();
    ...
    // The same journey as a TREE: the path now appears twice more — once in the
