...
5. Update the corresponding WorkOS allowed callback to `https://<hostname>/api/auth/callback`, plus application/logout origins. Update that environment's GitHub App URLs: the Webhook URL (`/api/github/webhook`) and the Callback URL (`/api/github/callback`), with "Request user authorization (OAuth) during installation" enabled, the permissions Pull requests read (the events and the changed files) and Checks read and write (posting the check) beside Contents and Metadata, and the Pull request, Check run and Check suite events subscribed beside Push (the installation events arrive without a subscription) (an account that installed before a permission was added is asked by GitHub to accept it: without Pull requests read nothing is checked, and without Checks write its checks run here and are not posted). **Before the first deploy that carries pull request checks**, make sure no two workspaces have a Context source naming the same repository (`select repo_full_name from context_sources where repo_full_name is not null group by 1 having count(*) > 1`, or the same over `config->>'repoFullName'` on a database that predates the column): the migration refuses to apply over duplicates, naming them, and the boot stops — pick the owning workspace and remove the other source first. **One GitHub App per environment, never shared**: when the last workspace removes an account, the server uninstalls the App from that account on GitHub, so an App shared with another environment would lose that environment's access too. If GitHub refuses the uninstall (an outage, a permission it no longer grants), the account is removed here anyway and the person is told to uninstall it on GitHub by hand; nothing retries it.
