...
// RawData is one JSON record per line written by the VM's health timer:
// { timestamp, healthy, release }. Freshness uses the producer timestamp, so
// replaying buffered records after an outage cannot look like recovery.
var healthBase = '''
TrueCourseHealth_CL
...
var alerts = [
  ...
  {
    // Three consecutive minutes, so the restart inside a release never pages.
    name: 'application-unhealthy'
    description: 'The public health check has failed for three consecutive samples. Check service logs and database connectivity.'
    severity: 1
    query: '${healthQuery}\n| where observed > ago(10m)\n| top 3 by observed desc\n| summarize samples=count(), unhealthy=countif(coalesce(tobool(health.healthy), false) == false)\n| where samples == 3 and unhealthy == 3'
  }
  {
    name: 'memory-high'
