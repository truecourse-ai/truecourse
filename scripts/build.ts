...
// llm has no intra-workspace deps but core + spec-consolidator import it, so it
// must be built before them. Missing here, fresh-checkout `build:dist` failed at
// the esbuild bundle with "Could not resolve @truecourse/llm" (it only worked in
// a dirty tree where packages/llm/dist already existed).
