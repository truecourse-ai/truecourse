...
- **Pass 2 (missing)** re-prompts contracts with unresolved cross-references. A produced fix (the target, or a single fallback obligation explaining the gap) is accepted only if it parses; if nothing parses after the attempts, the reference is simply left unresolved — a soft issue, strictly better than injecting a body the validator would hard-drop.
