...
The *kind* an enumerator assigns is as load-bearing as the target's inclusion, because it is effectively **locked** downstream: generate emits one artifact of exactly that kind, and repair can only fix an artifact in place (same kind + identity) — neither stage can re-classify a target. A wrong kind is therefore unrecoverable — it's written as `.tc` that can't be valid for that kind and dropped as a hard validation issue (it shows up in `validationIssues`, never silently). To make that decision correctly, enumerate and extract share a single **per-kind capability sheet** (`KIND_CAPABILITIES` in `prompt.ts`) — the one source of truth for what each kind is *for* and, load-bearingly, what it *cannot* express or which parent it belongs to (e.g. "a `Fallback` is null/absent → default only, never a comparison/clamp; a pagination limit's default/clamp is a facet of the `PaginationContract`; an Entity field's attributes belong to the Entity"). Enumerate uses it to pick the kind; extract's grammar builds on the same semantics — so the two can't drift, and the kind decision is made *with* the constraints rather than guessed from a one-liner. (This exact gap bit once with a weaker extract model: a pagination limit-clamp enumerated as a standalone `Fallback`, which has no valid syntax for a numeric clamp; it was dropped even though the clamp was already covered by the pagination contract — the shared capability sheet is what closes it.)
