...
Every target is a `(kind, identity, optional hint)`. There are **18 enumerable kinds** — the full declarable set minus `UnenforceableObligation`, which generation only ever emits as a last-resort faithfulness fallback and never plans for up front. That closed set is **enforced in code**, not just asked for in the prompt: every returned target's kind is validated against the list (derived at runtime from the `KIND_CAPABILITIES` catalog lines, so a kind added to the catalog is accepted with no second list to update). A view that returns an invalid kind gets **one corrective re-ask** naming the offending kinds; whatever is still invalid after that is dropped — the valid targets are always kept (the re-ask's output is unioned with them, so a sloppy re-emission can never lose yield), and an invented kind never flows downstream. Most kinds identify a target by a short slug, but a few follow a stricter convention worth knowing: an `Operation` is `"METHOD path"` (e.g. `POST /api/orders`), an `Entity` or `Enum` is just its type name, and a `StateMachine` is `"Entity.field"` (e.g. `Order.status`). The full kind catalog is the typed artifact union defined in [§5 (the .tc language)](#tc-language).

The *kind* an enumerator assigns is as load-bearing as the target's inclusion, because it is effectively **locked** downstream: generate emits one artifact of exactly that kind, and repair fixes an artifact in place (same kind + identity) — neither stage may re-classify a target to another structural kind (classification authority stays upstream, with enumerate + reconcile, on purpose). There is exactly **one sanctioned escape hatch**: when a mis-kinded target's content has *no* legal encoding in the locked kind's grammar, the repair pass may accept a deliberate downgrade to an `UnenforceableObligation` — the one verdict every kind shares — re-keying the artifact (collision-guarded), so the obligation survives visibly in `unenforceable/` instead of the artifact being dropped as a hard validation issue (see [§4.8](#gen-tail)). To make the kind decision correctly in the first place, enumerate and extract share a single **per-kind capability sheet** (`KIND_CAPABILITIES` in `prompt.ts`) — the one source of truth for what each kind is *for* and, load-bearingly, what it *cannot* express or which parent it belongs to (e.g. "a `Fallback` is null/absent → default only, never a comparison/clamp; a pagination limit's default/clamp is a facet of the `PaginationContract`; an Entity field's attributes belong to the Entity; a `FieldExposure`'s only two channels are a query projection and an API response body — a value surfaced via response headers or service metadata is not one"). Enumerate uses it to pick the kind; extract's grammar builds on the same semantics — so the two can't drift, and the kind decision is made *with* the constraints rather than guessed from a one-liner. (This exact gap bit twice: a pagination limit-clamp enumerated as a standalone `Fallback` with no valid clamp syntax, and an "API version surfaced in service metadata and response headers" enumerated as a `FieldExposure` that no legal `via` channel could express — each incident's missing limit is now stated on its catalog entry, and the downgrade path keeps the next unforeseen one from silently dropping an artifact.)
...
Each area's enumerate result is content-addressed cached. The cache key folds in the fingerprint of the enumerate prompt, the area id, and a hash of every doc's reference and content — so the cache invalidates on any doc edit or rename in the area, an area-id change, or an edit to the prompt itself, and nothing else. A view whose LLM call fails (e.g. a timeout) is **retried once** before it counts as a loss — enumerate used to be the pipeline's only single-attempt stage (generate retries through its completeness rounds, repair through its bounded loop), and that asymmetry is exactly how a marginal timeout once cost two whole areas their contracts. A view that fails both attempts contributes no targets, and one bad view never aborts the whole run — the other areas still generate. But a failed view marks the *area* as failed, which has three deliberate consequences: (1) the area's result is **not cached** — caching a partial or empty enumeration after a timeout would poison the key, so a re-run would return the empty list as a hit and never retry; (2) the area is **left out of the manifest**, so the next run treats it as changed and re-attempts it rather than no-opping past it; and (3) its id is reported in the result's `enumerateFailures`, and the driver prints a loud `WARNING` naming the areas to re-run. This is the safety net for the one failure mode the coverage-gap count can't see (below).

This phase uses the `sonnet` tier with a 300-second per-call timeout — a slow model/proxy can run an enumerate prompt in 140–180s+, so the ceiling is set well above that. For models/proxies that are systematically slower still (a gpt-5.5-via-proxy run was observed at p50 86s with the big areas blowing past 300s), every per-call timeout in the pipeline can be widened uniformly with `TRUECOURSE_LLM_TIMEOUT_SCALE` (a float multiplier applied at the transport, default 1 — see [§8.13](#llm-orchestration)); relative per-stage ceilings are preserved. It reports its per-area target count to the dashboard as each area finishes.
...
It works in two layers. First, a deterministic de-dup collapses targets that already share a coverage key, remembering for each one the **lexicographically smallest** area id that enumerated it — a deterministic home, so a shared target's owner can't flip when area order changes. Then, only when reconciliation is enabled and at least two distinct targets remain, the semantic layer runs — but **not** as one global call. Targets are grouped into small deterministic candidate clusters (same kind, sharing a meaningful slug token); a cluster needs at least two members to reach the LLM at all, and each cluster is reconciled by its own call and cached on its own members. Before any cluster reaches the model, three deterministic rules collapse the obvious duplicates: identical token multisets merge for every kind (`error.envelope.standard` ↔ `standard-error-envelope`); and for the four cross-cutting **singleton kinds** (auth requirement, error envelope, pagination, idempotency) a token-subset pair merges (`bearer-api` ⊂ `auth.bearer.api`) and a cluster containing exactly one identity that already exists in the **prior (anchored) contracts** collapses onto that incumbent. The canonical pick is fully deterministic — incumbent first, then the most-token (most specific) identity, then lexicographically smallest — and when a merge renames an identity, every fragment's cross-references to the old spelling are rewritten to the canonical at the assemble tail, so a cache-hit area can't ship a dangling reference. Deterministic merges are never written to the cluster cache; the LLM judges only what remains. Within a cluster the model proposes semantic merges — things that are the same concept under different spellings — and picks one canonical `(kind, identity)` per merge group. The prompt gives worked examples (merging `outbox-pattern` with `transactional-outbox`, `bearer-jwt` with `customer-bearer-jwt`), spells out what is *not* a duplicate (different kinds, operations that differ by method or path, entities that differ by name), and — because the cross-cutting kinds duplicate the most and mis-merging is worse than missing a merge — lands on the rule **"when unsure, do not merge."**
...
- **Every fragment is parse-checked on arrival,** under the same strict ohm grammar repair and validation use as their oracle. A fragment whose `tcSource` parses counts as produced; one that doesn't is *not* counted — its target stays missing, and its parser error is recorded.

After a round, the loop compares what was emitted against the enumerate checklist and re-prompts only the *misses* in a focused follow-up round — up to the retry budget (2 by default). This diff-and-retry is the **completeness gate**, and it now feeds errors back: a target whose previous fragment failed to parse is re-requested **with that parser error attached to its line** in the batch prompt, so the model sees exactly what it got wrong instead of being re-asked blind. The stop condition is a progress *signature* (parsing-fragment count plus each pending target's current parser error), not a bare count — a round that produced only unparseable fragments still changes the signature (there's a new error to feed back) and earns another round, while a round that reproduced the identical failing output stops the loop; the round budget bounds it either way. Two floors hold the line: a target that never yields a parsing fragment keeps its **last unparseable version anyway** (flushed at the end so the repair pass downstream still gets its shot — the parse gate adds earlier, cheaper feedback and never reduces yield), and whatever was produced — even partial coverage — is cached, so a re-run won't silently recover the misses unless the docs or targets actually change, which would bust the key anyway.
...
4. **Canonicalize cross-references.** Deterministic, run *before* repair so the LLM only sees what stays dangling. First, references to reconcile-**merged** non-canonical identities are rewritten to the canonical spelling ([§4.6](#gen-reconcile)); then any remaining dangling reference is **snapped** onto a declared identity of the same kind by a resolution ladder — coverage-key fold (benign drift the resolver's exact match misses), then folded token-multiset equality, then a unique meaningful-token overlap — snapping only when a *single* declared candidate matches, so the observed `Entity:core.customers` → `Entity:Customer` case resolves while an ambiguous or wrong-kind ref, and any forward ref to an unmodelled/undeclared kind (`PerformanceSLA:…`), is left verbatim. Each snap is surfaced as a soft note ([§12.10](#gaps-generate)).
5. **Repair.** An optional LLM pass that tries to fix broken or incomplete contracts (below). Skipped on a dry run, since it spawns the LLM.
6. **Validate.** A no-LLM parse-and-resolve gate, run through the verifier's own parser and resolver.
7. **Drop the unsalvageable.** Artifacts with hard validation issues are dropped and the survivors kept. A hard resolver error that can't be pinned to any specific artifact is escalated as a corpus-level failure.
...
**Repair** is a three-pass LLM step, and all three passes share one bounded re-prompt loop: feed the model the fresh parser error each attempt, and **accept a fix only if its `tcSource` actually parses** — never swap one broken body for another.

- **Pass 1 (malformed)** takes syntactically-broken contracts; the cheap `repairParse` (sonnet) tier handles the early attempts and the strong `repair` (opus) tier the last-ditch one (that escalation is this pass's alone — the other two are generation work and run the strong model on *every* attempt). A contract that still won't parse isn't discarded here — it's tagged with a failure reason and left for the validator to drop later, so the reason survives. This pass also owns the **one sanctioned kind change** in the pipeline: when the model answers that a clause has no legal encoding in the artifact's kind at all (its instruction explicitly invites this instead of inventing syntax), a parsing `UnenforceableObligation` is accepted as a deliberate downgrade — the artifact is re-keyed to the new kind/identity (guarded against colliding with an existing artifact, in which case it falls back to the tagged-for-validator path), so the obligation survives visibly instead of the artifact being dropped. No other kind change is ever accepted; classification stays upstream.
- **Pass 2 (missing)** re-prompts contracts with unresolved cross-references — the residue the deterministic snap (step 4) couldn't resolve. The re-prompt is told the **declared identities of the referenced kind**, so a reference that's really a mis-spelling of an existing artifact is fixed by declining to invent a duplicate rather than fabricating the target. A produced fix (the target, or a single fallback obligation explaining the gap) is accepted only if it parses; if nothing parses after the attempts, the reference is simply left unresolved — a soft issue, strictly better than injecting a body the validator would hard-drop.
- **Pass 3 (incomplete)** re-prompts ones that fail grammar-driven conformance checks, and replaces the previous body only with a parsing fix — if nothing parses, the previous (incomplete but valid) body is kept, since incomplete beats dropped.

Repair keeps the first slice on ties, which is one contributor to the origin-flip bug ([§4.12](#gen-nondeterminism)). The deep internals of merge, normalize, repair, and validate — and the `.tc` shapes they operate on — belong to [§5 (the .tc language)](#tc-language); here they're just the tail that turns raw fragments into validated artifacts.
...
Every timeout above is a *base* ceiling: the transport multiplies it by `TRUECOURSE_LLM_TIMEOUT_SCALE` (a float, default 1) at call time, so one knob widens every stage uniformly for a systematically slow model or proxy while preserving the relative per-stage ceilings (see [§8.13](#llm-orchestration)).
...
**Honest limit.** What the LLM writes for a genuinely new or edited area still varies until cached (the anchor dampens this; it can't eliminate it), and two residues remain open in [§12.10](#gaps-generate): repaired bodies aren't persisted back into the extract cache, and a cross-*reference* wrong in kind or ambiguous among same-kind candidates still ships as a soft unresolved diagnostic (the identity-drift case — an invented spelling of a declared identity — is now snapped deterministically at the assemble tail, [§4.8](#gen-tail)).
...
**The grammar also teaches the extractor.** A sibling module (`keyword-sets.ts`, next to the grammar) mechanically derives every **closed keyword set** from the grammar source — every rule whose alternatives lead with keyword literals (`FldExpChannel`, `OpFieldMod`, `EntFieldMod`, `FbkTrigger`, …), rendered one line per rule with each alternative's argument shape. The extraction system prompt embeds that rendering at module load ([§4.7](#generate)), so what the model is told it may write is derived from the same source the parser enforces — the two cannot drift, and a keyword added to the grammar reaches the prompt with no prompt edit. Rule selection is mechanical (no hand-curated list), which is the point: the reference covers the sets nobody has been burned by yet.
...
| `TRUECOURSE_LLM_TIMEOUT_SCALE` | float multiplier applied to every per-call timeout at the transport (slow model/proxy: `2`–`3`); invalid/≤0 falls back to 1 | `1` |
...
| `TRUECOURSE_LLM_TIMEOUT_SCALE` | Widen every stage's per-call timeout by a float multiplier (default 1) — for slow models/proxies that blow the tuned-for-Claude ceilings. |
...
| `pending_baselines` | one row per repo: the newest baseline request that lost the single-flight race, replayed when the running baseline settles ([§11.6](#ee-edition)) |
...
| `gh_prs` | per-PR state (`open`/`closed`/`merged`) + title + head sha, upserted from every pull_request webhook — feeds the Pulls page's Open/Closed filter (pre-feature rows have no entry and read as open) |
...
There is no separate "refresh contracts" job. When a dashboard decision resolves the last spec conflict, contract regeneration is a **forced re-baseline** of the current head (which regenerates contracts, recomputes the drift baseline, and re-verifies open PRs) — the re-curate itself already ran in the decision request. That way the user sees one progress panel (the baseline's), not a redundant wrapper. That flow is the repo scope; a PR-scoped decision edit that clears that PR's last conflict instead enqueues a `pr.regate` **job** — a forced re-gate of that single pull request, reusing the same synthesized-event mechanism the open-PR re-verify uses, but durable like the baseline: a jobs row, single-flight per repo + PR, orphan cleanup on boot, and a completion/failure notification so the resolving user sees the outcome.

How the jobs behave:

- **One definition, one envelope.** Every job type is declared through a single job-definition harness — its display title, its step checklist (seeded in full at start, so the panel shows the pending steps immediately), and its success/failure notification wording — and one executor owns the whole lifecycle: status transitions, SSE progress, notifications, error capture. The progress panel reads the title off the job record itself, so a job cannot ship with a nonstandard experience.
...
- **One of a kind at a time per repo, with coalescing.** Connecting, pushing, and chained refreshes all try to enqueue the same job; a "single-flight" guard keeps one baseline running per repo. A baseline request that loses the race isn't dropped — it's recorded as the repo's **pending follow-up** (latest commit wins, `force` preserved), and when the running baseline settles (success or failure) exactly one follow-up run is enqueued at that newest commit, unless it would repeat the commit that just ran without `force`. If the server restarts mid-job, leftover jobs are marked failed on boot and any pending follow-ups are drained back into the queue.
...
| **Cross-references wrong in KIND, or ambiguous among same-kind candidates, still ship as a soft unresolved diagnostic** (the *identity-drift* part is closed) | the generate + repair path (residual dangling refs are still only flagged by the `contract-verifier` resolver, later, at verify) | *Mostly closed.* The observed case — a regenerated `Order` emitting `field customerId … references Entity:core.customers` while the customer entity is declared `entity Customer` — is now caught deterministically. At the assemble tail, after the reconcile-merge rewrite and **before repair**, `snapReferencesToDeclared` resolves every dangling reference against the set's declared identities via a three-rung ladder (coverage-key fold → folded token-multiset → unique meaningful-token overlap), rewrites the referring side onto the declared identity with the token-accurate rewriter, and emits a soft note per snap; because it runs before repair, no LLM re-prompt is spent generating an artifact that already exists (§4.8). What can't be snapped falls through to repair's missing pass, now told the **declared identities of the referenced kind** so it can decline to invent a duplicate. **Residue:** a reference wrong in KIND (points at the wrong artifact type), or ambiguous among *multiple* same-kind candidates (two declared identities share a token, none is a multiset match), is deliberately left un-snapped — snapping only on a unique candidate — and still ships as a non-blocking soft *unresolved reference*. Forward refs to unmodelled or as-yet-undeclared kinds (`PerformanceSLA:…`) pass through untouched, by design | Widen disambiguation (slice-text co-occurrence, or a kind-corrective repair) for the ambiguous/wrong-kind residue; today it stays a tolerated soft diagnostic |
...
| **Absence drifts leak the temp contracts dir into PR output** | the gate verify runner's path rewrite | A drift with no code site (e.g. a spec'd route missing from code) cites its `.tc` file as the location — which in EE is the store's temp materialization directory, so check summaries and comments render `../tc-contracts-<rand>/….tc` instead of a stable contracts-relative path. The repo-relative rewrite covers only the clone tree; the contracts dir sits outside it |
...
1. **Determinism residue** — the generate-side fix has landed ([§4.12](#gen-nondeterminism)); what remains is the corpus side (cold re-scan variance, [§12.5](#gaps-nondeterminism)) and the generate residue (unpersisted repairs, and the ambiguous/wrong-kind dangling cross-references the deterministic snap leaves untouched — [§12.10](#gaps-generate)), which is what the **spec diff** ([§12.3](#gaps-spec-diff)) still rides on.
