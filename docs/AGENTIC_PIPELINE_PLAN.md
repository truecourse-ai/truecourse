...
and `guard generate`. Companion to `docs/SPEC_GUARD_PLAN.md` (which stays
the record for the deterministic planning and verification layers this plan
keeps).

Provenance: supersedes `docs/GUARD_AGENT_PLAN.md` (PR #870, the reference
implementation); THIS plan restarts the implementation from scratch with the
widened scope. The guard-generate design below carries over as the seed; the
spec-scan and guard-setup phases are to be designed (placeholder sections at
the end).

STATUS: document under extension — spec-scan and guard-setup phases are
being added; do not start implementation from this document yet.
...
## 12. Spec scan — agentic phases (TO BE DESIGNED)

Placeholder: the corpus-path scan (relevance, area-tags, overlap, chains,
curation) re-shaped as agent sessions. To be extended.

## 13. Guard setup — agentic phases (TO BE DESIGNED)

Placeholder: recipe discovery/verification, externals detection and skeleton,
seeding — re-shaped as agent sessions with the executor as their tool. To be
extended.
