...
6. **Battle-test fixes** (docs.strapi.io, 283 pages) — STATUS: BUILT. Two defects, both fixed at the root:
   ...
   - **Estimate presentation, split from the run checklist (as-built follow-up).** A leading `Estimating cost` step made the terminal checklist paint TWICE (once during estimation with every real step pending, again after the confirm), because the stdout renderer redraws the whole list in place. The estimate now reports through its own narrow seam — `EstimatePhase { start/done/error }`, passed as `onEstimatePhase` to `curateInProcess` / `guardGenerateInProcess` (`withEstimatePhase` in `progress.ts` replaces `withEstimateStep`); the CLI renders it as a clack spinner resolving to `Cost estimated — N docs` above the estimate panel (the `claude`-preflight shape) so the checklist starts with the run itself and paints once, and the dashboard opts back in as a leading checklist step via the `estimateStepPhase(tracker)` adapter (its popup replaces in place).
